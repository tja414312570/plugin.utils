...
public class PackageScanner {
	...
	public void doScanner(final ClassInter inter) {
		...
		for (String packDir : packageDirs) {
			...
			String packReplace;
			int i = 0;
			if (new File(packDir).isFile()) {
				packDir += "!";
			}
			if ((i = packDir.indexOf("!")) > -1) {
				String file = packDir.substring(0, i);
				String path = packDir.substring(i + 1);
		...
		scanner.scanner(new ResourceInter() {
			...
			public void find(Resource resourceEntry) {
				if (resourceEntry.getName().indexOf("$") == -1) {
					String resourcePath = ResourceManager.processPath(resourceEntry.getPath());
					String className = resourcePath.replace(classPath, "").replace("/", ".").replace(".class", "");
					className = className.substring(0, 1).equals(".") ? className.substring(1, className.length())
							: className;
					try {
						inter.find(ClassLoader.getSystemClassLoader().loadClass(className));
					} catch (Throwable e) {
						if (!ignoreLoadingException)
							throw new RuntimeException("failed to load class \"" + className + "\" at class file \""
									+ resourceEntry.getPath() + "\"", e);
					}
				}
